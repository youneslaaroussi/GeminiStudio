...
const fetchJobPayload = async (token) => {
...
const waitForFonts = async () => {
    const link = document.querySelector('link[href*="fonts.css"]');
    if (link) {
 ...
 */
const loadCompiledProject = async () => {
    ...
    try {
        ...
        if (!project || typeof project !== 'object') {
            throw new Error('Compiled project did not set globalThis.__SCENE_PROJECT__ within timeout');
        }
        return project;
    }
    finally {
...
const attachRendererHooks = (renderer, totalFrames) => {
...
const run = async () => {
    ...
    const segmentEnd = segmentEndParam !== null ? Number(segmentEndParam) : payload.output.range[1];
    ...
    const totalFrames = Math.round(Math.max(0, payload.output.range[1] - payload.output.range[0]) * payload.output.fps);
