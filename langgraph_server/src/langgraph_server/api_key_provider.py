...
def _get_keys_list(settings: Settings) -> list[str]:
    """Resolve list of API keys from settings. Prefer GEMINI_API_KEYS; fallback to GOOGLE_API_KEY."""
...
def is_quota_exhausted(exc: BaseException) -> bool:
    """Return True if the exception indicates Gemini quota exhausted (429)."""
    msg = str(exc).upper()
    if "429" in msg or "RESOURCE_EXHAUSTED" in msg or "QUOTA" in msg:
        return True
    # LangChain wraps Google errors
    if hasattr(exc, "message"):
        return "429" in str(getattr(exc, "message", "")) or "RESOURCE_EXHAUSTED" in str(
            getattr(exc, "message", "")
        )
    return False
