...
def _build_status_model_for_id(settings: Settings, api_key: str, model_id: str) -> ChatGoogleGenerativeAI:
...
async def _call_model(
    ...
    status_model_ids = settings.status_model_ids
    if not status_model_ids:
        logger.warning("GEMINI_STATUS_MODEL / GEMINI_STATUS_MODEL_IDS not set; status messages will be empty")
        return None
    n_keys = max(1, keys_count())
    last_exc: BaseException | None = None
    for model_idx, model_id in enumerate(status_model_ids):
        if model_idx > 0:
            logger.info("[STATUS] Trying model %s (fallback %d)", model_id, model_idx + 1)
        for _ in range(n_keys):
            api_key = get_current_key()
            if not api_key:
                logger.warning("No Gemini API key; status messages will be empty")
                return None
            model = _build_status_model_for_id(settings, api_key, model_id)
            try:
                if context == "thinking":
