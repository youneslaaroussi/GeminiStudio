...
def generateImage(
    ...
    # Call Gemini API
    api_url = f"https://generativelanguage.googleapis.com/v1beta/models/{settings.banana_model}:generateContent?key={settings.google_api_key}"
    ...
    if response.status_code != 200:
        ...
        logger.error("[BANANA] API error: %s - %s", response.status_code, error_text)
        return {
            "status": "error",
            "message": f"Gemini API error (HTTP {response.status_code}): {error_text}",
            "reason": "api_error",
        }
