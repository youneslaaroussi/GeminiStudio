...
![Gemini 3](https://img.shields.io/badge/Gemini-3-4285F4?logo=google&logoColor=white)
...
- [Gemini 3: The Reasoning Layer](#gemini-3-the-reasoning-layer)
...
This isn't a chatbot wrapper. The agent has **real agency**: it calls the renderer, manipulates the timeline, triggers Veo/Lyria/Chirp generation, and proactively notifies you when your video is ready. Gemini 3 becomes the **reasoning layer** for the entire production stack.
...
## Gemini 3: The Reasoning Layer

Gemini 3 isn't just integrated—it's the brain that makes agentic video possible.

**Agent Brain (Gemini 3 Pro Preview)**
Every interaction flows through Gemini 3. It reasons over project state, decides which tools to invoke, and orchestrates the entire editing pipeline. Without Gemini 3's reasoning and tool use, there is no execution layer—only a traditional UI waiting for human input.

**Multimodal Understanding (Files API)**
The agent doesn't just receive text—it *sees* and *hears*. Media uploads go through the Gemini Files API for deep multimodal analysis, producing rich semantic descriptions that power intelligent search and context-aware editing decisions.

**Asset Intelligence Pipeline (Google Cloud Video AI)**
Every uploaded asset goes through a full perception pipeline:
- **Shot Detection** — Automatic scene boundary identification
- **Label Detection** — Object and context recognition throughout the video
- **Speech Transcription** — Full audio-to-text with word-level timestamps
- **Gemini Analysis** — High-level semantic understanding ("what's happening here?")
...
**Generative Pipeline (Veo 3, Image Gen, TTS)**
The agent doesn't just edit existing footage—it creates. Need b-roll? It generates via Veo 3. Need a thumbnail? Gemini image generation. Need narration? TTS on demand. These aren't add-ons; they're first-class tools the agent invokes autonomously based on narrative intent.

**The Stack:**
| Layer | Role |
|-------|------|
| **Gemini 3** | Reasoning + tool orchestration |
| **Files API** | Multimodal perception |
| **Video Intelligence API** | Shot detection, labels, transcription |
| **Veo 3 / Imagen / Chirp** | Generative media creation |
...
**1. Agent Receives Intent** — User speaks naturally (web or Telegram). The Gemini 3 agent parses the request and plans the execution.
